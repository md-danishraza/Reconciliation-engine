...
export class IngestionService {
  ...
  static async ingestFromCSV(
    ...
    for (let i = 0; i < parseResult.data.length; i++) {
      ...
      const validation = DataQualityService.validateTransaction(
        row,
        source,
        rowNumber
      );
      ...
      const normalizedType =
        validation.isValid && validation.cleanedData.type
      ...
      if (validation.cleanedData.transactionId) {
        isDuplicate = await DataQualityService.detectDuplicate(
          validation.cleanedData.transactionId,
          validation.cleanedData.timestamp,
          validation.cleanedData.asset,
          validation.cleanedData.quantity || 0,
      ...
      if (isDuplicate) {
        result.duplicates++;
        logger.warn(
          `Duplicate transaction detected in ${source}:`,
      ...
      try {
        ...
        if (!validation.isValid) {
          ...
          logger.warn(
            `Saved invalid transaction from row ${rowNumber}:`,
            validation.issues
          );
        }
   ...
   */
  private static normalizeAsset(asset: string): string {
   ...
   */
  static async clearSource(source: "user" | "exchange"): Promise<void> {
    const result = await Transaction.deleteMany({ source });
    logger.info(
      `Cleared ${result.deletedCount} transactions for source: ${source}`
    );
  }
  ...
  static async getStats(): Promise<{
    ...
    userWithIssues: number;
    exchangeWithIssues: number;
    userInvalidCount: number; // NEW
    exchangeInvalidCount: number; // NEW
  }> {
