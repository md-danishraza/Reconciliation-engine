...
//  Checks each transaction for issues like missing timestamps, negative quantities, invalid types, etc.
export interface ValidationResult {
  isValid: boolean;
  issues: DataQualityIssue[];
  cleanedData: Partial<ParsedTransaction>;
}
...
export class DataQualityService {
  static validateTransaction(
    ...
    const cleanedData: Partial<ParsedTransaction> = {
      source,
      originalRow: row,
      dataQualityIssues: [],
    ...
    const hasCriticalErrors =
      hasTimestampError || hasQuantityError || hasTypeError || hasAssetError;

    // Store all issues as strings
    cleanedData.dataQualityIssues = issues.map(
      (i) => `${i.field}: ${i.issue} (${i.severity})`
    );
