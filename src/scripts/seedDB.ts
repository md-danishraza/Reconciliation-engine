...
async function seedDatabase() {
  try {
    ...
    const userResult = await IngestionService.ingestFromCSV(
      userFilePath,
      "user"
    );
    ...
    const exchangeResult = await IngestionService.ingestFromCSV(
      exchangeFilePath,
      "exchange"
    );
    ...
    if (userResult.qualityIssues.length > 0) {
      ...
      if (userResult.qualityIssues.length > 5) {
        console.log(
          `      ... and ${userResult.qualityIssues.length - 5} more issues`
        );
      }
    }
    ...
    console.log(
      `   Exchange transactions with issues: ${stats.exchangeWithIssues}`
    );
    console.log(
      `   Invalid user transactions (not matchable): ${stats.userInvalidCount}`
    );
    console.log(
      `   Invalid exchange transactions (not matchable): ${stats.exchangeInvalidCount}`
    );
