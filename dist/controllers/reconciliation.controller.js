...
export class ReconciliationController {
     ...
     */
    static async startReconciliation(req, res) {
        try {
            const config = {
     ...
     *         description: Internal server error
     */
    static async getFullReport(req, res) {
        try {
            ...
            for (const match of run.matches) {
                ...
                const exchangeTx = await Transaction.findById(match.exchangeTransactionId);
                reportEntries.push({
     ...
     *         description: Reconciliation run not found
     */
    static async getUnmatched(req, res) {
        try {
            let { runId } = req.params;
            if (Array.isArray(runId)) {
                runId = runId[0]; // take the first element
            ...
            res.json({
                runId: run.runId,
                totalUnmatched: run.summary.unmatchedUser + run.summary.unmatchedExchange,
                unmatched,
            });
        }
        catch (error) {
