...
export class MatchingService {
     ...
     */
    static calculateMatchScore(userTx, exchangeTx, config) {
        ...
        const timestampDiffSeconds = Math.abs(userTx.timestamp.getTime() - exchangeTx.timestamp.getTime()) /
            1000;
        // Check if within tolerance
        const isTimestampWithinTolerance = timestampDiffSeconds <= config.timestampToleranceSeconds;
        // 4. Calculate quantity difference percentage
        const quantityDiffPercent = Math.abs(((userTx.quantity - exchangeTx.quantity) / exchangeTx.quantity) * 100);
        // Check if within tolerance
        const isQuantityWithinTolerance = quantityDiffPercent <= config.quantityTolerancePct;
        ...
        if (isTimestampWithinTolerance) {
            ...
            score +=
                0.1 *
                    Math.max(0, 1 - timestampDiffSeconds / (config.timestampToleranceSeconds * 10));
        }
        ...
        if (isQuantityWithinTolerance) {
            ...
            score +=
                0.1 *
                    Math.max(0, 1 - quantityDiffPercent / (config.quantityTolerancePct * 10));
        }
     ...
     */
    static async findBestMatch(userTx, exchangeTransactions, config) {
        const candidates = [];
        for (const exchangeTx of exchangeTransactions) {
            // Skip if already matched
            if (exchangeTx.isMatched)
                continue;
     ...
     */
    static isConflictingMatch(match, config) {
        const isTimestampOutside = match.timestampDiffSeconds > config.timestampToleranceSeconds;
        const isQuantityOutside = match.quantityDiffPercent > config.quantityTolerancePct;
     ...
     */
    static async runMatching(runId, config) {
        ...
        logger.info(`Found ${userTransactions.length} user transactions and ${exchangeTransactions.length} exchange transactions to match`);
        const matches = [];
        ...
        for (const userTx of userTransactions) {
            ...
            if (bestMatch && bestMatch.score > 0.6) {
                ...
                if (!matchedExchangeIds.has(exchangeId)) {
                    ...
                    await Transaction.findByIdAndUpdate(bestMatch.exchangeTransaction._id, {
                        isMatched: true,
        ...
        const unmatchedExchangeIds = exchangeTransactions
            ...
            .map((tx) => tx._id);
        logger.info(`Matching completed: ${matches.length} matches, ${unmatchedUserIds.length} unmatched user, ${unmatchedExchangeIds.length} unmatched exchange`);
        ...
        logger.info(`Perfect matches: ${perfectMatches.length}, Conflicting matches: ${conflictingMatches.length}`);
        return {
            matches,
            unmatchedUserIds,
